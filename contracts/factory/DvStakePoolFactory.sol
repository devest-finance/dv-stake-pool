...
contract DvStakePoolFactory is IDvFactory, Ownable {
    ...
    function detach(address payable _tokenAddress) external payable onlyOwner {
        DvStakePool token = DvStakePool(_tokenAddress);
        token.detach();
    }
    ...
    */
    function issue(address _tokenAddress, string memory name, string memory symbol) public payable isActive
    {
        ...
        // issue
        DvStakePool token = new DvStakePool(_tokenAddress, name, symbol, address(this), _msgSender());
